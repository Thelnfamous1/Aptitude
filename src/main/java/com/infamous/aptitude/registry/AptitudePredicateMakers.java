...
public class AptitudePredicateMakers {
    ...
    public static final RegistryObject<Codec<PiglinlikeWantsToStopFleeing>> PIGLINLIKE_WANTS_TO_STOP_FLEEING = register("piglinlike_wants_to_stop_fleeing", () ->
            RecordCodecBuilder.create(builder -> builder.group(
                    CodecUtil.defineFieldUnchecked(ForgeRegistries.MEMORY_MODULE_TYPES.getCodec(), "target_memory", PiglinlikeWantsToStopFleeing::targetMemory),
                    CodecUtil.defineFieldUnchecked(PredicateMaker.DIRECT_CODEC, "target_predicate", PiglinlikeWantsToStopFleeing::targetPredicate),
                    CodecUtil.defineFieldUnchecked(BiPredicateMaker.DIRECT_CODEC, "fallback_entity_target_bipredicate", PiglinlikeWantsToStopFleeing::fallbackEntityTargetBiPredicate)
            ).apply(builder, PiglinlikeWantsToStopFleeing::new)));
