...
import com.infamous.aptitude.logic.function.VecThreeAtBottomCenterOf;
...
public record PathfinderSetWalkTargetAwayFromMaker<T>(MemoryModuleType<T> targetMemory, float speedModifier, int farEnough, boolean overrideWalkTarget, FunctionMaker<T, Vec3> positionGetter) implements BehaviorMaker {
    ...
    public static PathfinderSetWalkTargetAwayFromMaker<BlockPos> pos(MemoryModuleType<BlockPos> targetMemory, float speedModifier, int farEnough, boolean overrideWalkTarget) {
        return new PathfinderSetWalkTargetAwayFromMaker<>(targetMemory, speedModifier, farEnough, overrideWalkTarget, (FunctionMaker<BlockPos, Vec3>) (Object) new VecThreeAtBottomCenterOf());
    }
